...
```
Take a look at these data. What is this new column `mc1_pre_tf`?

Your turn. Write the same code, except mutating `mc1_post` to create a TRUE/FALSE (i.e. correct/incorrect) variable.
```{r}
...
But wait, there's more.  One of the most powerful features of the `summarize()` function is that you can group by other variables as well.  In this case, you might be interested in whether we see similar improvements for students taking both courses, or if more students improved in one of the courses.  
